...
app.post("/register", async (req, res) => {
  ...
  if (!gender) {
    return res
      .status(400)
      .json({ success: false, message: "성별을 선택해주세요." });
  }
        ...
        db.query(
          ...
          (err) => {
            if (err) {
              ...
              return res
                .status(500)
                .json({ success: false, message: "회원가입 실패" });
            }
...
app.get("/api/user-info", async (req, res) => {
  ...
  try {
    ...
    if (userRows.length === 0) {
      return res
        .status(404)
        .json({ message: "해당 닉네임의 유저를 찾을 수 없습니다." });
    }
...
app.put("/api/user/:id/profile", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db
      .promise()
      .query("SELECT * FROM users WHERE nickname = ? AND user_id != ?", [
        nickname,
        userId,
      ]);
...
app.post("/api/review", async (req, res) => {
  const { title, content, ratings, nickname, movie_id, image, category } =
    req.body;
  ...
  try {
    ...
    const [userRows] = await db
      .promise()
      .query(
        "SELECT user_id, points, grade_code FROM users WHERE nickname = ?",
        [nickname]
      );
    ...
    await db
      .promise()
      .query("UPDATE users SET points = ?, grade_code = ? WHERE user_id = ?", [
        updatedPoints,
        new_grade_code,
        user_id,
      ]);
...
app.post("/api/review/:id/views", async (req, res) => {
  ...
  try {
    await db
      .promise()
      .query("UPDATE review SET views = views + 1 WHERE review_id = ?", [
        reviewId,
      ]);
...
app.post("/api/review/:id/like", async (req, res) => {
  ...
  try {
    ...
    const [rows] = await db
      .promise()
      .query("SELECT * FROM likes WHERE review_id = ? AND user_id = ?", [
        reviewId,
        userId,
      ]);
    ...
    if (rows.length > 0) {
      ...
      await db
        .promise()
        .query("DELETE FROM likes WHERE review_id = ? AND user_id = ?", [
          reviewId,
          userId,
        ]);
...
app.get("/api/review/:id/likes", async (req, res) => {
  ...
  try {
    const [rows] = await db
      .promise()
      .query("SELECT COUNT(*) AS likeCount FROM likes WHERE review_id = ?", [
        reviewId,
      ]);
...
app.delete("/api/review/:id", async (req, res) => {
  ...
  try {
    ...
    await db
      .promise()
      .query("DELETE FROM comment WHERE review_id = ?", [reviewId]);

    // 2. 좋아요 삭제
    await db
      .promise()
      .query("DELETE FROM likes WHERE review_id = ?", [reviewId]);

    // 3. 리뷰 평점 항목 삭제 (review_rating 테이블도 있을 경우)
    await db
      .promise()
      .query("DELETE FROM review_rating WHERE review_id = ?", [reviewId]);
