...
function EditProfile() {
  ...
  const [originalNickname, setOriginalNickname] = useState(""); // 상단 표시용
  ...
  try {
    await axios.put(`http://localhost:4000/api/user/${storedUser.user_id}/profile`, {
      nickname,
      age,
      gender
    });
    ...
    // 로그인 페이지로 이동
    navigate("/login"); // 또는 "/LoginMain" 등 로그인 경로에 맞게 수정
  } catch (err) {
    console.error("프로필 저장 오류:", err);
    alert("프로필 저장 실패: " + (err.response?.data?.message || "알 수 없는 오류"));
  }
};
